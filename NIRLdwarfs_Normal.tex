...
Optimal spectral classification of L dwarfs is necessary to make effective use of the data being used to resolve the current discrepancies in optical and near-infrared spectral types, address the complexities in spectral morphology, and provide a consistent interpretation of underlying physical properties. 
Toward this goal, we use new and extant optical and NIR spectra to compile a sample of 180 L dwarfs with spectral coverage from 0.6-–2.4$\micron$. 
We present new spectra for 10 low gravity L dwarfs for a total sample of 29 $\beta$ and $\gamma$ objects.
We also present new low-resolution NIR spectra for \# field objects for a total sample of \#.
...
These templates reveal that the optical types are correlated with near-infrared spectral morphologies when analyzed using individually normalized bands and also show the range of spectral morphologies spanned by each spectral type. 
...
% Want to cite: \citet{Cushing05}: spectral sequence at moderate resolution.
% Want to cite: \citet{Cushing05}: spectral sequence at moderate resolution. \citet{Cushing08, Witte11}: model fits
...
%Our NIR follow-up campaign was focused on candidate nearby ultracool dwarfs that were too faint for optical follow-up.
...
Spectral average templates are a common technique used in stellar and extragalactic spectroscopy to reveal the most common spectral morphologies, average out any defects, and to provide data with high signal-to-noise ratios.
...
\begin{enumerate} \itemsep1pt \parskip0pt \parsep0pt
	\item a field gravity sample with 145 objects that are similar to the \citet{K99} field spectral standards.
...
The spectra are normalized band-by-band and combined using the weighted average flux at each pixel.
...
A $\chi^2$ cutoff of two balances our desire to include more spectra than exclude from the templates while also excluding objects with peculiar spectral morphologies.
...
There are only four L1$\gamma$ objects and the iterative $\chi^2$ method is ineffective at identifying \object[2M0227-1624]{2M0227-1624} as an obvious outlier. 
Based on a by-eye analysis, this one object was excluded from the template calculation.
...
As yet, our optical-NIR sample does not include an L2$\beta$ and only includes one each of L3$\beta$ and L4$\beta$, \object[2Mblah]{2Mblah} and \object[2Mblah2]{2Mblah} \citep{Cruz09_lowg}.
...
Our goal is to create spectral average templates that are representative of the field population of L dwarfs and which reflect intrinsic spreads in spectral morphology due to the age and metallicity spread of the thin disk.
However, the statistics and biases at each spectral type are different and the resulting templates likely do not represent the same segment of the bulk population.

The spectral type distribution of the optical-NIR sample is shown in Figure~\ref{fig:spthist}. 
The field gravity objects that are included in the templates are shown as black bars while the excluded objects are gray bars.
At each type, there are at least eight objects included in the template.
However, at the earlier types (L0--L3), more objects were considered and a higher fraction were rejected compared to the later types (L4, L6--L8).
With more objects, the $\chi^2$ rejection method is more effective at identifying subtle outliers.
This suggests that the later type templates could potentially reflect a wider range of spectral morphologies compared to the earlier types.


something different between L3 and L4 related to number of objects.


In Figure~\ref{fig:JK_colors}, we show the 2MASS $J-K_S$ color versus spectral type distribution for the optical-NIR sample of L~dwarfs. The plot shows the colors of the individual L dwarfs (filled circles) as well as the average color at each spectral type (black bar). All of the objects have reliable 2MASS photometry with uncertainties smaller than 0.1~mag. 
...
excluded/peculiar spectra are not necessarily color outliers. 

dispersion among color of objects with typical spectral morphorolgy

middle 50\% much tighter for L0-L3 compared to L4-L8. Not likely due to small numbers, because L4 and L5 have lots of objects.
maybe dust turning on or seeing range of underlying masses, ages, gravities in the brown dwarfs compared to the stellar dominated earlier types (~70 M_Jup)
...
\begin{figure}
	\epsscale{0.25}
		...
		\plotone{/Users/kelle/Dropbox/Analysis/NIRtemplates/opt/1093.pdf} %0210
		...
		\plotone{/Users/kelle/Dropbox/Analysis/NIRtemplates/opt/985.pdf}
...
\begin{figure}
	...
	\emph{Bottom}: Same data as in top panel but normalized and plotted band-by-band. The spectra are normalized individually by band (optical, $zJ$, $H$, and $K$ bands) using nearly the entire band range. Prominent atomic and molecular features are indicated, as well as telluric correction artifacts due to poor line scaling of the A0 star's Hydrogen lines.}
	\label{fig:L1fan}
\end{figure}
