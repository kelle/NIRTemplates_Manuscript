...
The resulting spectral average templates of field gravity objects are shown in Figure~\ref{fig:field_templates} and discussed in \S~\# while the low-gravity templates are in Figures Figures~\ref{fig:L0lg}--\ref{fig:L5lg} and discussed in \S~\#. The templates (black lines), are calculated using the weighted average flux at each pixel. 
...
\textbf{something about the L8/L9 issue.}
...
The features contributing to a good match in the $K$ band are both the blue and red slopes on either side of the 2.13~$\micron$ peak.	
	...
	\item As is common practice, spectral types based on low signal-to-noise data ($S/N\sim15$) can be indicated with a `:' suffix or `::' for very-low signal-to-noise ($S/N\sim5$) data (e.g., \citet{Sanduleak:1988fn} ) For example, `L1:' could be used to describe a low signal-to-noise spectrum that matches the L1 template best in both $J$ and $H$ bands. `L1--L3::' could be used to describe a very low signal-to-noise spectrum that matches the L1, L2, and L3 templates all equally well.

As is common practice, a colon is used to indicate increased uncertainty in the assignment of the spectral type. See, for example, Sanduleak & Pesch (1988), Silvestri et al. (2006), Gray & Corbally (2009; Table 12.1), and Covey et al. (2010)."
