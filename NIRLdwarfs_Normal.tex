...
\title{
Meeting the Cool Neighbors XII: Optically-Anchored Analysis of the Near-Infrared Spectra of L Dwarfs.
}
...
L dwarfs were first classified according to their spectral morphology at red-optical wavelengths using spectra typically covering 6000--9000~\AA at $R~1000$ \citep{K99,Kirkpatrick05}. 
...
That is, we observe a wide range of spectral morphologies in the NIR for sources with similar optical spectral types.

% relevant work on NIR spectral classification
While there were numerous efforts to classify L dwarfs with spectral indices \citep{Reid01_NIR, Testi01, Geballe02}, a classification system for L~dwarfs based on NIR data grounded in the Morgan Keenan system did not emerge until recently. 
\citet{Kirkpatrick10} (hereafter K10) developed a classification scheme that focuses heavily on the $YJ$-band spectral morphology and results in types consistent with the optically-determined types. 
\citet{Kirkpatrick10} defined L dwarf near-infrared spectral standards based on their spectral morphology in the $J$-band. 
\citep{Allers13} defines quantitative spectral indices to identify spectral type and surface gravity class using low-resolution near-infrared spectra, but use of the entire spectral region to assign a type is necessary to parallel the MK system established for stars \citep{Morgan84}.
...
2) Much of the NIR data for L dwarfs has been collected with the SpeX spectrograph in prism mode, covering 0.8--2.5~$\micron$ at $R~200$, a far larger wavelength regime and lower resolution than the optical.
