...
%The saturation of the shading of the gray strip indicates the variance of the weighted average flux at each pixel. Darker gray indicates smaller variance, more agreement amongst the spectra and thus a more reliable template at that wavelength. 
...
\item the presence of \ion{Na}{1} (1.14, 2.2~$\micron$) becomes less common (although the absorption feature at 2.2~$\micron$ is due to only a small subset of objects used to calculate the template), and
...
As discussed by \citet{Kirkpatrick05,Kirkpatrick10}, several NIR spectral classification schemes for L dwarfs have been in use but they rely on spectral indices and/or do not use the entire NIR spectral regime. The \citet{Geballe02} system uses only one water index for L dwarf classification. The K10 system provides spectral standards but relies heavily on the $J$~band for classification. Most recently, \citet{Allers:2013hk} proposed an index-based scheme that distinguishes objects with different gravities but requires both $J$-~and $H$-band data.
In an MK system \citep{Morgan:1984wy}, the ideal scheme uses the on-sky spectral standards and and the entirety of the spectrum to estimate a type. 
...
We propose a scheme that takes advantage of normalizing band-by-band (as described above in \S~\ref{sec:method}), uses spectral standards and the spectral average templates, and provides a method to assign spectral types to L~dwarfs with only $H$-~or $K$-band data. In principal, we could set up three spectral typing schemes, one for each band, but for simplicity, we propose that a range of optically-anchored spectral types be adopted for $H$ or $K$-band only data.
We do not advocate for $H$-~or $K$-band spectral types, but rather we provide a method for estimating a spectral type range when optical or $J$-band data is not available.
As described below, the $K$-band is the least diagnostic of spectral type and a spectral type based solely on $K$-band data will be the least accurate, with a typical range being three types. $H$-band data is slightly more diagnostic that $K$ and will narrow the spectral type range down but high signal-to-noise $J$ band data is necessary for a spectral type anchored to the optical type.
...
%We have shown that the K10 spectral standards are consistent with the templates based on objects all with the same optical type and thus presumably highly correlated with $M_J$ and temperature \citep[Fig.~7 \& 9]{Kirkpatrick05}.

%Below, we detail our recommended and idealized prescription for determining a spectral type for an L dwarf target from NIR spectra. Every observing program is different and we anticipate investigators will modify it for different circumstances.
...
\begin{itemize}
	\item The L2 NIR standard, \object{2MASS~1305$-$2541} Kelu-1, is a confirmed binary. Was rejected in H and K band, but not in J.
	\item The L5 NIR standard, \object{2MASS~0835+1953}, has no available optical spectrum. 
	\item The L7 NIR standard, \object{2MASS~0103+1935}, is classified as L6 in the optical (reference?), we have re-classified it as L6pec after a re-analysis of its optical spectrum. It's included in the L6 template.
\end{itemize}
...
Something about red and blue suffix.
...
\bibliography{testbib6}
%export from Papers. Entire, Standard, Abberviation
...
\begin{figure}
	\epsscale{1}
	\plotone{figures/fig_oplot_templates.pdf}
...
\begin{figure}
	...
	\caption{Normalized NIR range of 2MASS targets identified as not Ms, set 2 of 3. The objects plotted are: 055742-1333, 0558-1339, 1357-3946, 1506+2759, 1721-0619, and 2240+3848.}\include{tables/tab_normal_template.tex}
	\label{fig:notMs_2}
\end{figure}
