...
Includes Figures~\ref{fig:spthist}, \ref{fig:JK_colors}, \ref{fig:beta_templates}--\ref{fig:gamma_templates} 
Tables ~\ref{tab:lowg_template}, \ref{tab:lowg_excluded}.
...
The low-gravity sample is composed of objects from \citet{Cruz09_lowg} and the new data described in \SS~\ref{sec:obs_new_opt} and~\ref{sec:obs_new_nir}.
As described in \cite{Cruz09_lowg}, $\beta$ and $\gamma$ spectral type suffixes are used to indicate very low-gravity and intermediate-gravity spectral features respectively.

The sample of low gravity objects is small and incomplete but the striking homogeneity of the NIR spectral morphologies amongst objects of the same optical spectral type provided strong motivation for making templates.

Included in the \optLowG~objects are 23 L0--L4 objects with a $\gamma$ gravity classification and six L0--L1 $\beta$ type objects.
Of the~\optLowG, only three were excluded from the templates and all primarily due to the low signal-to-noise ratio of their NIR spectra, not because of their inconsistent spectral morphology (Table~\ref{tab:lowg_excluded}).
