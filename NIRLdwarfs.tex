...
\end{itemize}
The spectral differences between each of the low gravity templates and the field templates are discussed in detail in the following subsection.

\subsubsection{Low-gravity $\gamma$ and $\beta$ templates compared to the field gravity templates}
...
\label{sec:summary}
We have presented new low-resolution NIR SpeX spectra for \NewPrismObjects L dwarfs and moderate-resolution far-red spectra for \NewOptObjects.
Combining these new data with extant data, we have created a sample of \sample L dwarfs with both high quality optical and NIR spectra comprised of \optField field gravity L0--L5 objects and \optLowG L0--L4 low gravity $\beta$ and $\gamma$ objects.

In our analysis of NIR spectra, we normalize the spectra band-by-band rather than at one small wavelength window.
This normalization method effectively reduces the broad-band reddening term present in the spectra and facilitates the comparison of absorption features.
Further, we have analyzed the NIR spectra in our sample using the optical spectral type as an anchor.

We combined the relatively large number of optically-anchored L~dwarf NIR spectra at each spectral type to generate $zJ$, $H$, and $K$ band spectral average templates.

We expect that these templates encompass the range of spectral morphologies spanned by typical L dwarfs. 
This range is likely dominated by intrinsic properties, such as clouds, gravity, and metallicity, and not instrumental or atmospheric effects.
...
\begin{itemize}
	\item L dwarfs of the same optical spectral type do indeed have the common spectral morphologies in the NIR even though they span a wide range of colors.
	\item The $J-K$ color scatter amongst L dwarfs of the same optical spectral type is attributable to the spectra having different smooth functions across the entire 1--2.5~$\micron$ range rather than differences amongst individual absorption features.
	...
	\item Some objects excluded from the templates have near average colors. This implies that not all objects with peculiar spectra have have extreme colors.
	\item Some objects with colors in the red/blue tails of the color distribution are included in the templates. This implies that not all 
objects with extreme colors display any spectral peculiarities besides spectral slope.
	\item \textbf{Something about interquartile range}
While diversity of cloud properties is often pointed to as the primary suspect for extreme colors of low gravity objects, a significant amount of further theoretical and observational work is needed to pinpoint the nature of the physical properties modulating the $J-K_S$ colors of L dwarfs.
	
%It is not surprising that the low-gravity spectral features in the NIR are also more subtle than those present in the objects typed as $\gamma$ based on optical data.
%Data for more $\beta$ type objects and at higher spectral resolution data will likely provide more diagnostics for intermediate gravity objects.

We have used the spectral templates to evaluate the current suite of spectral standards. 
 we have suggested alternate standards where the standards have turned out to be non-representative of the population or where suitable standards exist which are brighter or at intermediate declination.
