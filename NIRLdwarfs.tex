...
Toward this goal, we use new and extant optical and NIR spectra to compile a sample of~\sample~L dwarfs with spectral coverage from 0.6-–2.4$\micron$, including \optLowG~low gravity $\beta$ and $\gamma$ objects.
We present \NewPrismSpectra~new low-resolution NIR spectra and ~\NewOptSpectra~new optical spectra.
We utilize a method for analyzing the NIR spectra of L dwarfs that partially removes the broad band spectral slope and reveals similarities in the absorption features between objects of the same optical spectral type.
...
These templates reveal that the optical types are correlated with NIR spectral morphologies when analyzed using individually normalized bands and show the range of spectral morphologies spanned by each spectral type.
...
We re-evaluate the NIR spectral standards proposed by \cite{Kirkpatrick10} and \cite{Allers:2013hk} using the templates and propose some new standards that are easily observable and representative of their subtypes.
Finally, we build on the spectral typing method of \cite{Kirkpatrick10} to provide a method for estimating spectral types for L dwarfs which is tied to the optical and can be used when only $H$- or $K$-band data are available.
The methods we present here provide resolutions to several long standing issues with classifying L dwarf spectra and 
could also be a first step in a spectral classification scheme for cloudy exoplanets.
\\
\\
\\
\end{abstract}
