...
\author{\textbf{Ellianna Schwab}} %nir spectral types comparsion
\affil{\textbf{Department of Physics, City College of New York, City University of New York, 160 Convent Avenue, New York, NY 10031}}
...
\begin{abstract}
\textbf{Discrepancies between competing optical and near-infrared (NIR) spectral typing systems for L dwarfs have motivated us to search for a classification scheme which ties the optical and NIR schemes and addresses complexities in the spectral morphology.}
...
The spectra are shown in Figure Set~\ref{fig:newopt} and are publicly available from the \textbf{BDNYC Database}.
...
These data are publicly available from the BDNYC Database.}
\figsetgrpend
...
\begin{figure}[b]
  ...
  \caption{New far-red optical spectra listed in Table~\ref{tab:newopt}. Unlike many spectra of L dwarfs, these data have been telluric corrected. These data are publicly available from the \textbf{BDNYC Database}. Additional images are available in the Figure Set in the electronic edition of the journal.\label{fig:newopt}}
\end{figure}
...
SpeX has proven to be a popular workhorse instrument for brown dwarf spectroscopic follow-up and, as a result, there is a plethora of data publicly available in SpeX Prism Library Analysis Toolkit (SPLAT)\footnote{SpeX Prism Library Analysis Toolkit (SPLAT): \url{http://www.browndwarfs.org/splat}} and its predecossor, the Spex Prism Library \footnote{Spex Prism Spectral Library: \url{http://pono.ucsd.edu/~adam/browndwarfs/spexprism/}} \citep{Burgasser:2014tr}. 
...
\textbf{These data are publicly available from both SPLAT and the BDNYC Database.}
...
The later-type $\gamma$ objects (e.g., \objectname[2MASS J03552337+1133437]{2M0355+1133}, \citealt{Cruz09_lowg}) display significant spectral diversity and we defer an analysis of these data to a future paper.
...
\textbf{Many of the objects in the optical-NIR sample have existing NIR spectral types in the literature; however, these types are based on several different schemes with inconsistent suffix usage.
In order to give a general sense of the pre-existing NIR types for objects in the optical-NIR sample, we have identified a subsample composed of 78 objects in the optical-NIR sample included in the SpeX Prism Library Analysis Toolkit (SPLAT)\footnote{SpeX Prism Library Analysis Toolkit (SPLAT): \url{http://www.browndwarfs.org/splat}} with a listed NIR spectral type.
...
\begin{figure}
  ...
  \caption{\textbf{Swarm plot representation of optical spectral types compared to literature NIR types from SPLAT for 78 objects in the optical-NIR sample.
...
\begin{itemize}
	\item If $JHK$ spectra are available, normalize the data at 1.28~$\micron$ and compare to the NIR spectral standards as described in K10. Note if the \textbf{spectral slope is unusually red or blue compared to the standard.}
	...
	\textbf{If multiple templates fit equally well in the \emph{zJ} band, then look for the best fit in the \emph{H} band.
...
\textbf{As described in  \S~\ref{sec:sample}, we used SPLAT to identify NIR spectral types existing in the literature for 78~objects in the optical-NIR sample.
...
% ***Something about the six outlier objects. Issue #20

\begin{figure}
  \plotone{nir_field_obj.pdf}
  \caption{\textbf{Swarm plot representation of literature NIR spectral types compared to NIR types found using the method described here.
  Filled orange, blue, and pink circles indicate objects where the suffixes agree while a black star indicates suffix disagreement.
  While the agreement is generally good, with 58/78~(74\%) agreeing within one subtype, we note a strong tendency for the literature types to be later than the ones found with the method proposed here.}}
...
\begin{figure}
  ...
  \caption{\textbf{Swarm plot representation of optical spectral types compared to NIR types found using the method described here.
  Filled orange, blue, and pink circles indicate objects where the suffixes agree while a black star indicates suffix disagreement.
  In general, the agreement is excellent, with 90\% of NIR types being within one subclass of the optical type and having the same suffix (153/\sample).
  Only 6/\sample~(4\%) objects are off by more than one subtype and 13/\sample~(8\%) were assigned a different suffix.}}
...
\textbf{These data are all are publicly available from the BDNYC Database.}
...
The scheme
\begin{itemize}
  \item \textbf{results in spectral types consistent with optical types,}
