...
Finally, we build on the work of \cite{Kirkpatrick10} to provide a spectral typing method which is tied to the optical and can be used when only $H$- or $K$-band data are available.
...
\citet{Kirkpatrick10} developed a classification sequence based on spectral standards which hinges on the $zJ$-band spectral morphology and uses the $H$ and $K$ bands to assign 'red' or 'blue' suffixes based on the overall spectral slope.
...
\cite{Geballe02} define the start of the T dwarf sequence as the first appearance of the 1.6$\micron$ methane absorption feature in the $H$~band.
...
\item $H$ band slopes get steeper and shape becomes more pointed,
...
\item red side of the $H$ band slope is steeper resulting in triangular shaped $H$ band,
\item 1.6~$\micron$ FeH absorption is not present, and
\item blue side of the $K$ band slope is steeper.
\end{itemize}
...
\emph{On the basis of these data, we conclude that in order to use low-resolution NIR data to credibly distinguish an L0$\gamma$ from a field object, a $K$-band spectrum is insufficient and $H$- and/or $zJ$-band data are required.}
}
...
\emph{On the basis of these data, we conclude that in order to use low-resolution NIR data to credibly distinguish an L0$\beta$ from a field object in the NIR, $zJ$ and/or $K$-band data are insufficient and $H$-band data are required.}
}
...
\emph{On the basis of these data, we conclude that in order to use low-resolution NIR data to credibly distinguish an L1$\gamma$ from a field object, a $K$-band spectrum is insufficient and $H$- and/or $zJ$-band data are required.}
}
...
\emph{On the basis of these data, we conclude that in order to use low-resolution NIR data to credibly distinguish an L4$\gamma$ from a field object in the NIR, $zJ$ and/or $K$-band data are insufficient and $H$-band data are required.}
}
\end{description}
