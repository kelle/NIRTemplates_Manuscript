%!TEX root = /Users/kelle/Dropbox/Pubs IN PROGRESS/Paper XII NIR DATA/Cruz-Nunez/NIRLdwarfs.tex
\begin{deluxetable}{ll}
%\tabletypesize{\scriptsize}
%\tabletypesize{\tiny}
\tablecolumns{2}
\tablecaption{New Near-Infrared IRTF SpeX Prism Observations\label{tab:newnir}}
\tablehead{
\colhead{} & \colhead{Obs Date (UT)}   \\
\colhead{Designation} &  \colhead{yyyy/mm/dd}
}
\startdata
%00100009$-$2031122 & 2008/07/14 \\
00250365+4759191 & 2003/09/03 \\
00325584$-$4405058 & 2007/11/13 \\
00325584$-$4405058 & 2008/09/07 \\
00332386$-$1521309 & 2003/09/04 \\
\enddata
\tablecomments{Data are available \added{as Data behind the Figure} and from SPLAT and the BDNYC Database. This table is published in its entirety in a machine readable format in the electronic edition of the journal. A portion is shown here for guidance regarding its form and content.}
\end{deluxetable}
